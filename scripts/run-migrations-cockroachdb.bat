...
echo ════════════════════════════════════════════════
echo.
echo [INFO] Running migrations for CockroachDB...
echo [INFO] Database URL: %DATABASE_URL%
echo [INFO] Using standardized migration files
echo.
...
:: 使用sqlx migrate run
echo [INFO] Executing migrations (this may take 10-30 seconds)...
sqlx migrate run --database-url "%DATABASE_URL%"
set MIGRATION_EXIT=%ERRORLEVEL%

if %MIGRATION_EXIT% EQU 0 (
    echo.
    echo [OK] ✅ Migrations completed successfully!
    echo.
    echo [INFO] Standardized migration files executed:
    ...
    echo.
    echo [WARN] Migration failed (exit code: %MIGRATION_EXIT%)
    ...
    echo [TIP] You can also run migrations manually: sqlx migrate run --database-url "%DATABASE_URL%"
