# 🚀 IronCore 企业级部署指南

**版本**: 1.0.0  
**更新日期**: 2025-12-02  
**部署模式**: 生产就绪（包含所有P0修复）

---

## ✅ 前置要求

### 1. 系统要求
- **操作系统**: Linux (Ubuntu 22.04+) / macOS / Windows with WSL2
- **CPU**: 4核以上
- **内存**: 8GB以上
- **磁盘**: 50GB以上 SSD

### 2. 软件依赖
```bash
# Rust 1.75+
curl --proto '=https' --tlsv1.2 -sSf https://sh.rustup.rs | sh

# PostgreSQL 14+ 或 CockroachDB 23+
# Redis 7.0+
# Docker & Docker Compose（推荐）
```

---

## 📦 快速启动（Docker Compose）

### 1. 克隆仓库
```bash
git clone <repository-url>
cd Rust-Blockchain/IronCore
```

### 2. 配置环境变量
```bash
cp .env.example .env
vim .env
```

**必须修改的配置**:
```bash
# 数据库连接
DATABASE_URL=postgresql://user:password@localhost:26257/ironcore

# Redis连接
REDIS_URL=redis://localhost:6379

# JWT密钥（生成随机64字符）
JWT_SECRET=$(openssl rand -hex 32)

# 钱包主密钥（生成随机64字符）
WALLET_MASTER_KEY=$(openssl rand -hex 32)
```

### 3. 启动基础设施
```bash
# 启动 PostgreSQL + Redis
docker-compose up -d postgres redis

# 等待服务就绪
sleep 10
```

### 4. 运行数据库迁移
```bash
# 安装 sqlx-cli
cargo install sqlx-cli --no-default-features --features postgres

# 运行迁移
sqlx migrate run
```

### 5. 初始化默认配置
```bash
# 编译并运行
cargo build --release
./target/release/ironcore
```

### 6. 验证部署
```bash
# 健康检查
curl http://localhost:8088/api/health

# 查看Swagger文档
open http://localhost:8088/swagger-ui
```

---

## 🔧 手动部署（生产环境）

### 步骤1: 安装数据库

#### 选项A: PostgreSQL
```bash
# Ubuntu
sudo apt update
sudo apt install postgresql-14 postgresql-contrib

# 创建数据库
sudo -u postgres psql
CREATE DATABASE ironcore;
CREATE USER ironcore WITH PASSWORD 'your-secure-password';
GRANT ALL PRIVILEGES ON DATABASE ironcore TO ironcore;
\q
```

#### 选项B: CockroachDB（推荐）
```bash
# 下载
wget https://binaries.cockroachdb.com/cockroach-v23.1.0.linux-amd64.tgz
tar xvzf cockroach-v23.1.0.linux-amd64.tgz
sudo cp cockroach-v23.1.0.linux-amd64/cockroach /usr/local/bin/

# 启动单节点
cockroach start-single-node --insecure --listen-addr=localhost:26257 --http-addr=localhost:8080 --background

# 创建数据库
cockroach sql --insecure -e "CREATE DATABASE ironcore;"
```

### 步骤2: 安装Redis
```bash
# Ubuntu
sudo apt install redis-server

# 启动
sudo systemctl start redis-server
sudo systemctl enable redis-server

# 验证
redis-cli ping
```

### 步骤3: 编译Rust项目
```bash
cd IronCore-V2

# 添加Redis依赖（如果还没添加）
cargo add redis tokio-redis regex

# 编译（Release模式）
cargo build --release
```

### 步骤4: 运行迁移
```bash
sqlx migrate run
```

### 步骤5: 初始化费率配置
平台费用规则会在服务启动时自动初始化/补齐（确保 `/api/v1/fees/calculate` 可用）。

### 步骤6: 启动服务
```bash
# 前台运行（测试）
./target/release/ironcore

# 后台运行（生产）
nohup ./target/release/ironcore > ironcore.log 2>&1 &
```

---

## 🎯 验证部署

### 1. 基础健康检查
```bash
# 服务健康
curl http://localhost:8088/api/health

# 响应: "OK"
```

### 2. 创建测试钱包
```bash
curl -X POST http://localhost:8088/api/v1/wallets/batch \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer YOUR_JWT_TOKEN" \
  -d '{
    "wallets": [
      {
        "chain": "ETH",
        "address": "<CLIENT_DERIVED_ADDRESS>",
        "public_key": "<CLIENT_DERIVED_PUBLIC_KEY>",
        "derivation_path": "m/44\x27/60\x27/0\x27/0/0",
        "curve_type": "secp256k1",
        "name": "Test ETH Wallet"
      }
    ]
  }'
```

> 注意：后端不派生地址；`address/public_key` 必须来自客户端派生。

### 3. 测试费率计算
```bash
curl -X POST http://localhost:8088/api/v1/fees/calculate \
  -H "Content-Type: application/json" \
  -d '{
    "fee_type": "SwapServiceFee",
    "amount_usd": 1000,
    "chain": "ethereum"
  }'
```

### 4. 验证钱包列表
```bash
curl -H "Authorization: Bearer YOUR_JWT_TOKEN" "http://localhost:8088/api/v1/wallets"
```

---

## 🔒 安全检查清单

部署前必须完成：

- [ ] 修改所有默认密钥和密码
- [ ] 配置防火墙（只开放必要端口）
- [ ] 启用HTTPS（Nginx反向代理）
- [ ] 配置日志脱敏
- [ ] 启用风控中间件
- [ ] 配置备份策略
- [ ] 设置监控和告警
- [ ] 进行渗透测试
- [ ] 获得安全审计报告

---

## 📊 生产环境架构

```
                        ┌─────────────────┐
                        │   Nginx/Caddy   │
                        │  (HTTPS + CORS) │
                        └────────┬────────┘
                                 │
                ┌────────────────┼────────────────┐
                │                │                │
           ┌────▼────┐     ┌────▼────┐     ┌────▼────┐
           │ IronCore │     │ IronCore │     │ IronCore │
           │ Instance │     │ Instance │     │ Instance │
           │    #1    │     │    #2    │     │    #3    │
           └────┬────┘     └────┬────┘     └────┬────┘
                │                │                │
                └────────────────┼────────────────┘
                                 │
                ┌────────────────┼────────────────┐
                │                │                │
           ┌────▼────┐     ┌────▼────┐     ┌────▼────┐
           │  Redis  │     │  PgSQL  │     │  ImmuDB │
           │ (Locks) │     │  (Data) │     │ (Audit) │
           └─────────┘     └─────────┘     └─────────┘
```

---

## 🔄 后台服务

### 自动启动的服务
1. **交易监控服务** (Transaction Monitor)
   - 间隔: 30秒
   - 功能: 回填gas_used, 更新确认数

2. **交易自动恢复** (Auto Recovery)
   - 间隔: 5分钟
   - 功能: 检测卡住交易，自动RBF

3. **跨链事件监听** (Cross-Chain Listener)
   - 间隔: 30秒
   - 功能: 监听源链和目标链状态

### 查看后台服务状态
```bash
# 查看日志
tail -f ironcore.log | grep -E "(Transaction monitor|Auto-recovery|Cross-chain)"
```

---

## 🐛 故障排查

### 问题1: 无法连接数据库
```bash
# 检查数据库是否运行
pg_isready -h localhost -p 26257

# 检查连接字符串
psql $DATABASE_URL
```

### 问题2: 无法连接Redis
```bash
# 检查Redis是否运行
redis-cli ping

# 检查端口
netstat -an | grep 6379
```

### 问题3: Nonce冲突
```bash
# 检查Redis锁
redis-cli KEYS "nonce_lock:*"

# 清除所有Nonce锁（紧急）
redis-cli --scan --pattern "nonce_lock:*" | xargs redis-cli DEL
```

### 问题4: 编译错误
```bash
# 清理并重新编译
cargo clean
cargo build --release

# 检查依赖
cargo tree
```

---

## 📈 性能优化

### 1. 数据库连接池
```bash
# 增加连接数
DATABASE_MAX_CONNECTIONS=50
```

### 2. Redis持久化
```bash
# redis.conf
appendonly yes
appendfsync everysec
```

### 3. 增加工作线程
```bash
# 设置Tokio线程数
TOKIO_WORKER_THREADS=8
```

---

## 🔄 升级步骤

```bash
# 1. 备份数据库
pg_dump ironcore > backup_$(date +%Y%m%d).sql

# 2. 停止服务
kill $(cat ironcore.pid)

# 3. 拉取新代码
git pull

# 4. 运行新迁移
sqlx migrate run

# 5. 编译
cargo build --release

# 6. 启动
./target/release/ironcore
```

---

## 📞 支持

- **文档**: 查看 `docs/` 目录
- **Issues**: GitHub Issues
- **Swagger**: http://localhost:8088/swagger-ui

---

**部署完成！系统现在是企业级生产就绪状态。**

